...
def stream_audio():
    ...
    try:
        ...
        yt_cmd = [
            'yt-dlp',
            '-f', 'bestaudio/best', # Fallback to video if audio-only isn't pipeable
            '-o', '-',
            '--quiet',
            '--no-warnings',
            video_url
        ...
        def generate():
            ...
            try:
                while True:
                    ...
                    # Smaller initial chunks to reduce Time-To-First-Byte
                    read_size = 1024 * 4 if chunks_sent < 10 else 1024 * 32

                    chunk = p2.stdout.read(read_size)
                    if not chunk:
                        break

                    if chunks_sent == 0:
                        logger.info("🎵 Stream started flowing (First chunk sent)")

                    yield chunk
                    chunks_sent += 1

                # Check for errors if no data was sent
                if chunks_sent == 0:
                    logger.error("❌ Stream Finished with 0 bytes sent")
                    ...
                    if p2.poll() is not None and p2.returncode != 0:
                        err = p2.stderr.read().decode()
